...
- Tests must be deterministic — no reliance on the machine's timezone, clock, or ambient env vars. Stub what you need (`vi.stubEnv`, `vi.setSystemTime`) and clean up afterwards.
...
Config lives in `vitest.config.mts`:

- `environment: "node"` — no DOM, because nothing under test needs one
- `resolve.tsconfigPaths: true` — the `@/*` alias comes from `tsconfig.json`, one source of truth
- `include: ["src/{actions,lib}/**/*.test.ts"]` — the scope above is enforced by the runner, so a component test cannot silently join the suite. Widening this glob is a deliberate decision, not a convenience.

`src/lib/utils.test.ts` is the reference example for the pattern.
